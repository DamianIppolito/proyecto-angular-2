...
export class RestauranteService{
  ...
  getRestaurante(id: string, random = null){
    if(random == null){
      ...
      return this._http.get("http://localhost:90/slim/restaurantes-api.php/random-restaurante").map(res => res.json());
    }
  }
  ...
  deleteRestaurante(id:string){
    return this._http.get("http://localhost:90/slim/restaurantes-api.php/delete-restaurante/"+id).map(res => res.json());
  }
}
